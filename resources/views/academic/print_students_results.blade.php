...
function viewStudentResults(){var c=$('#classSelect').val(),e=$('#examSelect').val(),y=$('#yearSelect').val();if(!c||!e||!y){alert('Select all');return;}$('#resultBody').html('<tr><td colspan="8" style="text-align:center;padding:20px;">Loading...</td></tr>');$.get('/api/student-results-summary',{class:c,examtp:e,acyear:y},function(r){var t=$('#resultBody');t.empty();if(r.success&&r.data.length>0){r.data.forEach(function(b,i){t.append('<tr><td>'+(i+1)+'</td><td>'+b.reg+'</td><td>'+b.sname+'</td><td>'+b.total+'</td><td>'+b.avg+'</td><td>'+b.pos+'</td><td>'+b.division+'</td><td><button onclick="printReportCard(\''+b.reg+'\',\''+e+'\',\''+y+'\',\''+c+'\')" style="background:#FF9900;color:#fff;border:none;padding:4px 12px;border-radius:4px;font-size:0.65rem;cursor:pointer;">Print</button></td></tr>');});}else{t.html('<tr><td colspan="8" style="text-align:center;padding:20px;">No results</td></tr>');}});}
function printReportCard(reg,exam,year,cls){window.open('/api/print-report-card?reg='+reg+'&exam='+exam+'&year='+year+'&class='+cls,'_blank');}
</script>
@endsection
