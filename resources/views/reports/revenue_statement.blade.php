...
@section('title', 'Revenue Statement')
@section('styles')
...
<script>
    $(document).ready(function() {
        $('.search').select2({ placeholder: "Search staff...", allowClear: true, theme: "default" });
        loadStaffList();
    });
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    } else { options = '<option value="">No staff found</option>'; }
                    select.html(options); select.trigger('change');
            ...
            error: function(xhr) {
                ...
                select.html('<option value="">Error loading staff</option>'); showAlert(errorMsg, 'error');
            }
        });
    }
    ...
    $('#viewStaffBtn').on('click', function() {
        ...
        if (!staffName) { showAlert('Please select a staff member.', 'error'); return; }
    ...
    function loadStaffStudents(staffName) {
        ...
        $.ajax({
            url: '/api/staff-students-list', type: 'GET',
            headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' },
            data: { staff: staffName, _token: '{{ csrf_token() }}' },
            success: function(response) {
                ...
                if (response.success) {
                    if (response.success) {
                        ...
                        $('#staffIdDisplay').text(response.staff.id || '-'); $('#staffStatusDisplay').text(response.staff.status || 'Active'); $('#staffInfo').show();
                    }
            ...
            error: function(xhr) {
                ...
                if (xhr.responseJSON && xhr.responseJSON.message) { errorMsg = xhr.responseJSON.message; }
                showAlert(errorMsg, 'error'); $('#emptyState').show();
            }
        });
    }
    ...
    function deleteStudent(reg) {
        ...
        $.ajax({
            url: '/api/student/delete', type: 'POST',
            headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' },
            data: { reg: reg, _token: '{{ csrf_token() }}' },
            success: function(response) {
                if (response.success) { showAlert(response.message, 'success'); const staffName = $('#staffSelect').val(); if (staffName) { loadStaffStudents(staffName); } }
                else { showAlert(response.message || 'Error deleting student', 'error'); }
            },
            error: function(xhr) {
    ...
    function showAlert(message, type) {
        const alert = $('#alertMessage'); alert.text(message); alert.removeClass('success error info').addClass(type).show();
